...
export default function Navbar({ services = [] }: { services?: any[] }) {
  ...
  // 🌟 โครงสร้างคอลัมน์ (เก็บไว้เหมือนเดิมเพื่อกำหนด Layout)
  const navColumns = [
                ...
                <div className="bg-white/95 backdrop-blur-xl border-t border-gray-100 shadow-[0_40px_60px_rgba(0,0,0,0.1)] w-full">
                  ...
                  <div className="max-w-7xl mx-auto px-8 py-12">
                    <div className="grid grid-cols-4 gap-12 normal-case tracking-normal">
                      ...
                      {/* 🌟 พระเอกของเรา: ลูปสร้างคอลัมน์อัตโนมัติ */}
                      {navColumns.map((column, colIndex) => (
                        <div key={`col-${colIndex}`} className="flex flex-col gap-8">
                          
                          {/* ลูปสร้างหมวดหมู่ในแต่ละคอลัมน์ */}
                          {column.map((categoryName) => {
                            // 🌟 กรองข้อมูล: ดึงเฉพาะบริการที่มี category ตรงกับหัวข้อปัจจุบัน
                            const categoryServices = services.filter((s: any) => s.category === categoryName);
                            ...
                            return (
                              <div key={categoryName}>
                                ...
                                <ul className="text-gray-500 text-sm font-medium space-y-3">
                                  ...
                                  {/* เช็คว่ามีบริการในหมวดหมู่นี้ไหม ถ้ามีให้ลูปสร้าง <li> */}
                                  {categoryServices.length > 0 ? (
                                    categoryServices.map((svc: any, i: number) => {
                                      // จัดการเรื่อง slug เผื่อกรณีข้อมูลมาจาก Sanity เป็น Object { current: '...' }
                                      const slugStr = typeof svc.slug === 'string' ? svc.slug : svc.slug?.current;
                                      
                                      return (
                                        <li key={`svc-${i}`} className="hover:text-gray-900 hover:translate-x-1 transition-all">
                                          <Link href={`/services/${slugStr}`} className="block w-full">
                                            • {svc.title}
                                          </Link>
                                        </li>
                                      );
                                    ...
                                    // ถ้ายังไม่มีบริการในหมวดนี้ ให้ขึ้นข้อความจางๆ
