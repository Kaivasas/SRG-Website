...
interface TestimonialCarouselProps {
  testimonialsData?: SanityTestimonial[];
}

export default function TestimonialCarousel({ testimonialsData = [] }: TestimonialCarouselProps) {
  const { baseData, displayData } = useMemo(() => {
    const base = testimonialsData.slice(0, 10);
    let display = [...base];

    if (base.length > 1 && base.length < MIN_DISPLAY_COUNT) {
      while (display.length < MIN_DISPLAY_COUNT) {
      ...
      <Reveal delayMs={200} className="w-full relative flex justify-center items-center">
        <div className="relative w-full h-[450px] md:h-[500px] flex justify-center items-center max-w-[1440px]">
          ...
          {displayData.map((current, i) => {
            ...
            if (total % 2 === 0 && diff === -(total / 2)) diff += total;
            ...
            const scaleClass = isCenter ? "scale-100" : "scale-[0.85]";
            const finalOpacityClass = isCenter ? "opacity-100" : Math.abs(diff) === 1 ? "opacity-40" : visibilityClass;
            const zIndexClass = isCenter ? "z-20" : Math.abs(diff) === 1 ? "z-10" : "z-0";
            const pointerEvents = isCenter ? "pointer-events-auto" : "pointer-events-none";
            ...
            return (
              ...
              >
                <div className="absolute -top-10 w-20 h-20 md:-top-12 md:w-24 md:h-24 rounded-full border-4 border-white shadow-xl overflow-hidden bg-gray-200">
                  <Image
                    src={current.avatar || FALLBACK_AVATAR}
            ...
            {baseData.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-500 ${
                  idx === originalActiveIndex ? "bg-white w-8" : "bg-white/30 w-2 hover:bg-white/60"
                }`}
