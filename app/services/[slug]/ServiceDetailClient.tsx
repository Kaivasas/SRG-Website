...
export default function ServiceDetailClient({ service }: { service: any }) {
  ...
  // คำนวณว่าตอนนี้ควรโชว์การ์ดใบไหน
  const totalCards = service.benefits?.length || 0;
  ...
  return (
    <div className="bg-transparent min-h-screen text-white font-sans selection:bg-blue-500 selection:text-white">
      ...
      {/* 4. Portfolio Section (ดึงข้อมูลของจริงจาก Works มาโชว์!) */}
        ...
        <section id="portfolios" className="py-24 relative z-10 bg-transparent overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 px-6 md:px-[10vw] w-full max-w-[1920px] mx-auto relative z-10">
            {service.portfolios.map((port:any , index:number) => (
              // ลิงก์ไปยังหน้าผลงานจริงๆ ได้เลย!
              <Link href={`/works/${port.slug}`} key={`${port._id}-${index}`} className="w-full group cursor-pointer rounded-2xl bg-white/5 backdrop-blur-xl p-6 border border-white/10 transition hover:bg-white/10 hover:border-blue-500/50 hover:shadow-[0_20px_60px_-15px_rgba(30,144,255,0.3)] flex flex-col shadow-inner shadow-white/5">
      ...
      <section className="py-32 px-6 bg-transparent relative z-10 overflow-hidden">
        <div className="max-w-7xl mx-auto text-center flex flex-col items-center relative z-10">
          ...
          <Link href="/contact" className="bg-white text-[#004965] font-black py-5 px-16 rounded-full text-xl shadow-[0_10px_30px_rgba(255,255,255,0.2)] hover:shadow-[0_15px_40px_rgba(255,255,255,0.4)] hover:scale-105 transition-all hover:bg-blue-50">
            ติดต่อเรา
          </Link>
        </div>
      </section>
